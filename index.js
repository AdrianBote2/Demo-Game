...
// Target Velocity for the Sandbox Mini-Game
...
modeButtons.forEach(mode => {
    document.getElementById(`mode-${mode}`).addEventListener('click', () => {
        ...
        const descriptions = {
            sandbox: `Endless Road: Match ${targetVelocity.toFixed(2)} m/s! (No Walls)`,
            challenge: "Target: Stay in the ring for 3s to reset the 10s timer!",
            racing: "Laps: Pass the Red Checkpoint, then the Green Finish!"
        };
...
function resetGame() {
    px = (currentMode === 'racing') ? 6 : 0; pz = 0; vx = 0; vz = 0;
...
function animate() {
    ...
    const tiltMag = Math.sqrt(radX**2 + radZ**2);
    const normalForce = mass * g * Math.cos(tiltMag);
    const maxFriction = mu * normalForce;

    // Determine if we overcome static friction
    ...
    // Tracers
    const accelX = nFX / mass;
    const accelZ = nFZ / mass;
    const instantAccel = Math.sqrt(accelX**2 + accelZ**2);
    const actualFriction = (nFX === 0 && nFZ === 0) ? Math.sqrt(fgX**2 + fgZ**2) : maxFriction;

    // Velocity & Position
    vx = (nFX === 0) ? vx * 0.92 : vx + accelX * 0.016;
    vz = (nFZ === 0) ? vz * 0.92 : vz + accelZ * 0.016;
    const instantVel = Math.sqrt(vx * vx + vz * vz);
    ...
    if (currentMode === 'sandbox') {
        const limit = 11;
        if (Math.abs(px) > limit) px = -Math.sign(px) * limit;
        if (Math.abs(pz) > limit) pz = -Math.sign(pz) * limit;
    } else {
    ...
    speedEl.innerText = instantVel.toFixed(2);
    document.getElementById('a-total').innerText = instantAccel.toFixed(2);
    document.getElementById('normal-force').innerText = normalForce.toFixed(2);
    document.getElementById('friction-force').innerText = actualFriction.toFixed(2);

    // Sandbox Match Feedback
    if (currentMode === 'sandbox') {
        if (Math.abs(instantVel - targetVelocity) < velocityTolerance) {
            speedEl.classList.add('speed-match');
        } else {
            speedEl.classList.remove('speed-match');
        }
    }

    // Challenge/Racing Logic
    if (currentMode === 'challenge') {
        ...
        raceTime += 0.016; document.getElementById('race-timer').innerText = raceTime.toFixed(1);
        ...
        if (pz > 5.8 && pz < 6.2 && hasCheckpoint) {
            ...
            if (laps >= 3) { if (raceTime < bestTime) bestTime = raceTime; document.getElementById('best-time').innerText = bestTime.toFixed(1); alert(`Finish: ${raceTime.toFixed(2)}s`); resetGame(); }
        }
    }
