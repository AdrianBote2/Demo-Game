...
function initThree() {
    ...
    // The Ball (Added to scene, NOT platformGroup)
...
function animate() {
    ...
    // 4. SURFACE SNAPPING (The Fix for phasing through floor)
    // We calculate the Y position based on the plane's rotation at the current X/Z
    const surfaceY = (pz * Math.sin(radZ)) - (px * Math.sin(radX));
    ball.position.set(px, surfaceY + 0.5, pz);
    ...
    if (currentMode === 'challenge') {
        ...
        if (timeLeft <= 0) { gameActive = false; alert("Final Score: " + score); location.reload(); }
        ...
        if (px > 6 && pz < -6) {
            ...
            alert("Goal Reached! Time: " + raceTime.toFixed(2) + "s");
            resetState();
        }
    }
